...
const GameLinks = <T extends Translation>({
  ...
  const submitAnswers = () => {
    ...
    for (let i = 0; i < gameState.answers.length; ++i) {
      const correctIndex = gameState.answers.findIndex(
        (x) => x.right == gameState.userAnswers[i].right
      );
    ...
    <>
      <Grid columns="3" gap={'4'} className="w-full">
        {gameState.entries.map((entry, entryIndex) => {
          const leftIndex = gameState.userAnswers.findIndex(
            (e) => e.left === entry.left && e.right != ''
          );
          const leftColour =
            gameState.selectedLeft == entry.left
              ? 'green'
              : leftIndex != -1
                ? backgroundColours[leftIndex]
                : undefined;
          const rightIndex = gameState.userAnswers.findIndex(
            (e) => e.right === entry.right
          );
          const rightColour =
            rightIndex != -1 ? backgroundColours[rightIndex] : undefined;
          const borderColour = gameState.showAnswers
            ? gameState.result[entryIndex]
              ? 'green-500'
              : 'red-500'
            : undefined;
          return (
      ...
      {gameState.userAnswers.filter((x) => x.right != '').length ==
